//
//  Tt3.UnitTests/TestBinaryFileWriterPerformanceAsBytes.cpp
//  
//  A test for the CBinaryFileWriter output performance (as bytes)
//
//////////
...
void TestBinaryFileWriterPerformanceAsBytes() throws(CTestException*)
{
    ...
    for (size_t iteration = 0; iteration < Iterations; iteration++)
    {
        ...
        CBinaryFileWriter w{ CString(tempFileName) };
        for (size_t i = 0; i < RandomDataSize; i++)
        {
            w.Write(randomChunk[i % RandomChunkSize]);
        }
        w.Close();
        remove(tempFileName);
    }
...
END_TT3_NAMESPACE
//  End of Tt3.UnitTests/TestBinaryFileWriterPerformanceAsBytes.cpp
