//
//  Tt3.UnitTests/TestFileOutputStreamConsistency.cpp
//  
//  A test for the binary output stream consistency
//
//////////
...
void TestFileOutputStreamConsistency() throws(CTestException*)
{
    ...
    for (size_t scan = 0; scan < RandomDataSize; )
    {
        ...
        fos1.Write(randomChunk, chunkSize);
        scan += chunkSize;
    }
    fos1.Close();
    ...
    for (size_t i = 0; i < RandomDataSize; i++)
    {
        ...
        {   //  OOPS!
            fclose(f1);
            remove(tempFileName);
            throw new CTestException("Reads a byte " + ToString(c1) +
    ...
    for (size_t i = 0; i < RandomDataSize; i++)
    {
        fos2.Write(randomChunk[i % RandomChunkSize]);
    }
    fos2.Close();
    //  ...and re-read the random data using CRT I/O and compare with the result
...
END_TT3_NAMESPACE
//  End of Tt3.UnitTests/TestFileOutputStreamConsistency.cpp
