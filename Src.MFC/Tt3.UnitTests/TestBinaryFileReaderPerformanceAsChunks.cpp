//
//  Tt3.UnitTests/TestBinaryFileReaderPerformanceAsChunks.cpp
//  
//  A test for the CBinaryFileReader performance (as chunks)
//
//////////
...
void TestBinaryFileReaderPerformanceAsChunks() throws(CTestException*)
{
    ...
    {
        CBinaryFileReader r{ CString(tempFileName) };
        for (size_t scan = 0; scan < RandomDataSize; )
        {
            size_t chunkSize = min(RandomChunkSize, RandomDataSize - scan);
            size_t bytesRead = r.Read(randomChunk, chunkSize);
...
END_TT3_NAMESPACE
//  End of Tt3.UnitTests/TestBinaryFileReaderPerformanceAsChunks.cpp
