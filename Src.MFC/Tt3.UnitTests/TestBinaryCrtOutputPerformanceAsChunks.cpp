//
//  Tt3.UnitTests/TestBinaryCrtOutputPerformanceAsChunks.cpp
//  
//  A test for the binary CRT output performance (as chunks)
//
//////////
...
void TestBinaryCrtOutputPerformanceAsChunks() throws(CTestException*)
{
    ...
    for (size_t iteration = 0; iteration < Iterations; iteration++)
    {
        ...
        for (size_t scan = 0; scan < RandomDataSize; )
        {
            ...
            size_t bytesWritten = fwrite(randomChunk, sizeof(char), chunkSize, f);
            if (bytesWritten != chunkSize)
            {   //  OOPS!
                fclose(f);
                remove(tempFileName);
                throw new CTestException("Attempting to write " + ToString(chunkSize) +
                                         " bytes, but only " + ToString(bytesWritten) +
                                         " bytes were written");
            }
            scan += chunkSize;
        }
...
END_TT3_NAMESPACE
//  End of Tt3.UnitTests/TestBinaryCrtOutputPerformanceAsChunks.cpp
