//
//  Tt3.UnitTests/TestBinaryFileWriterConsistency.cpp
//  
//  A test for the binary file writer consistency
//
//////////
...
void TestBinaryFileWriterConsistency() throws(CTestException*)
{
    ...
    CBinaryFileWriter w2{ CString(tempFileName) };
    for (size_t i = 0; i < RandomDataSize; i++)
    {
        w2.Write(randomChunk[i % RandomChunkSize]);
    }
    w2.Close();
    //  ...and re-read the random data using CRT I/O and compare with the result
...
END_TT3_NAMESPACE
//  End of Tt3.UnitTests/TestBinaryFileWriterConsistency.cpp
