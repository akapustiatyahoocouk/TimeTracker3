//
//  Tt3.UnitTests/TestBufferedInputStreamConsistency.cpp
//  
//  A test for the buffered input stream consistency
//
//////////
...
void TestBufferedInputStreamConsistency() throws(CTestException*)
{
    ...
    CBufferedInputStream bis1(&fis1);
    for (size_t scan = 0; scan < RandomDataSize; )
    {
        ...
        size_t bytesRead = bis1.Read(randomChunkCopy, chunkSize);
    ...
    CBufferedInputStream bis2(&fis2);
    for (size_t i = 0; i < RandomDataSize; i++)
    {
        int c2 = bis2.Read();
...
END_TT3_NAMESPACE
//  End of Tt3.UnitTests/TestBufferedInputStreamConsistency.cpp
