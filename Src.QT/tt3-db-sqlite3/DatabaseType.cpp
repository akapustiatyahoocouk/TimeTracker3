//
//  tt3-db-sqlite3/DatabaseType.cpp - tt3::db::sqlite3::DatabaseType class implementation
//
//  TimeTracker3
...
{
    static const QIcon icon(":/tt3-db-sqlite3/Resources/Images/Objects/SQLiteDatabaseTypeSmall.png");
    return icon;
}

QIcon DatabaseType::largeIcon() const
{
    static const QIcon icon(":/tt3-db-sqlite3/Resources/Images/Objects/SQLiteDatabaseTypeLarge.png");
    return icon;
}
...
{
    if (DatabaseAddress * sqlite3DatabaseAddress =
        dynamic_cast<DatabaseAddress*>(address))
    {   //  Address is of a proper type
        return new Database(sqlite3DatabaseAddress, Database::_OpenMode::_Create);
    }
    throw tt3::db::api::InvalidDatabaseAddressException();
}
    ...
    {   //  Address is of a proper type.
        switch (openMode)
        {
            case tt3::db::api::OpenMode::ReadOnly:
                return new Database(sqlite3DatabaseAddress,
            ...
            default:    //  be defensive!
                QFileInfo fileInfo(sqlite3DatabaseAddress->_path);
    ...
    )
{
    ...
    {   //  Address is of a proper type
        ...
        std::unique_ptr<Database> database
            { new Database(sqlite3DatabaseAddress, Database::_OpenMode::_OpenReadWrite) };
        database->close();
        QFile file(sqlite3DatabaseAddress->_path);
        if (!file.remove())
        {   //  OOPS!
...
//  End of tt3-db-sqlite3/DatabaseType.cpp
